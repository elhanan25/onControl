...
const AUTH_SECTIONS = ["loginSection", "registerSection", "forgotSection", "resetSection"];
function showSection(id) {
  AUTH_SECTIONS.forEach(s => document.querySelector(`#${s}`).classList.toggle("hidden", s !== id));
}

function showAuthOverlay(section) {
  els.authOverlay.classList.add("visible");
  els.appShell.classList.add("hidden");
  showSection(section === "register" ? "registerSection" : "loginSection");
}

els.showRegisterBtn.addEventListener("click", () => showSection("registerSection"));
els.showLoginBtn.addEventListener("click", () => showSection("loginSection"));

document.querySelector("#showForgotBtn").addEventListener("click", () => showSection("forgotSection"));
document.querySelector("#showLoginFromForgotBtn").addEventListener("click", () => showSection("loginSection"));
...
document.querySelector("#forgotForm").addEventListener("submit", async (event) => {
  ...
  try {
    ...
    msg.textContent = "אם הכתובת קיימת במערכת, יישלח אליה קישור לאיפוס הסיסמה.";
  } catch {
...
document.querySelector("#resetForm").addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (!response.ok) {
      msg.className = "form-message error";
      msg.textContent = data.error || "שגיאה באיפוס הסיסמה.";
      return;
    }
    msg.textContent = "הסיסמה אופסה בהצלחה! ניתן להתחבר כעת.";
    setTimeout(() => {
